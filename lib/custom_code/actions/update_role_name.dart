...
Future<void> updateRoleName(BuildContext context, String roleId) async {
  try {
    // Realiza una consulta a Firestore para obtener el documento de rol basado en roleId
    final DocumentSnapshot roleSnapshot =
    ...
    if (roleSnapshot.exists) {
      ...
      FFAppState().update(() {
        FFAppState().role = roleName;
      });
    } else {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('No se pudo encontrar el rol con el ID proporcionado.'),
        ),
      );
    }
  } catch (error) {
